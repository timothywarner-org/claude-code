...
<#
.SYNOPSIS
    Bulletproof, idempotent launcher for the Memory MCP server + standalone MCP
...
.DESCRIPTION
    ...
    half-dead Inspector, orphaned server). Two independent pieces:
      ...
      1. Memory MCP server  - `fastmcp.cli run server.py` on HTTP port 6280,
         reload OFF. Reload MUST stay off: fastmcp 3.3.1's reload watcher passes
         `--no-reload` to a subcommand that rejects it ("Unknown option"), so the
         server dies on spawn; reload also disables elicitation/sampling, which
         the delete_document and summarize_via_sampling demos need.
      2. MCP Inspector 1.0.0 - the standalone `@modelcontextprotocol/inspector`
         browser UI on port 6274 (proxy 6277). You connect it to the server via
         Transport = Streamable HTTP, URL = http://127.0.0.1:6280/mcp.

    Why NOT `fastmcp dev inspector server.py`: that wrapper is broken in fastmcp
    3.3.1 (it always forces --reload, which self-conflicts as above, and the
    fastmcp.exe console-script trampoline fails to canonicalize on this Python
    3.14 venv). Running the two pieces directly is the reliable path.

    This script lives in scripts/ but drives a server in
    segment_4_hero/memory_server/. It resolves that folder relative to its own
    location, so it runs from any cwd.

.PARAMETER ServerPort
    HTTP port for the memory server. Default 6280.

.PARAMETER UiPort
    Inspector UI port. Default 6274.

.PARAMETER ProxyPort
    Inspector proxy port. Default 6277.
...
.EXAMPLE
    ...
    http://localhost:6274. Then in the UI: Streamable HTTP ->
    http://127.0.0.1:6280/mcp -> Connect.

.NOTES
    Author: Tim Warner. Teaching artifact for "Context Engineering with MCP".
...
# This launcher lives in scripts/, but the server it drives lives in
# segment_4_hero/memory_server/. Resolve that server folder relative to THIS
# script so the launcher works from any cwd (Windows Terminal often starts in the
# user home dir, which is why a relative "server.py" fails).
$RepoRoot = Split-Path -Parent $PSScriptRoot
$ServerDir = Join-Path $RepoRoot 'segment_4_hero\memory_server'
...
if (-not (Test-Path $ServerDir)) {
    throw "Server folder not found at $ServerDir. Expected segment_4_hero/memory_server/ from this script."
}
...
#   - python running fastmcp.cli for THIS server
...
$orphans += Get-CimInstance Win32_Process -Filter "Name = 'python.exe'" -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -and ($_.CommandLine -match 'fastmcp\.cli|[\\/]server\.py') -and ($_.CommandLine -match $dirPattern) }
...
# --- Step 3: start the memory server (HTTP, no reload) ---------------------------
# Launched in a detached PowerShell child so this script can go on to run the
# Inspector in the foreground. The server inherits ANTHROPIC_API_KEY from env.
Write-Step "Starting memory server on $McpUrl (reload OFF - preserves elicitation)"
$serverArgs = @(
    '-m', 'fastmcp.cli', 'run', 'server.py',
    '--transport', 'http', '--port', "$ServerPort", '--no-banner'
)
$serverProc = Start-Process -FilePath $VenvPython -ArgumentList $serverArgs `
...
foreach ($i in 1..30) {
    ...
    try {
        # A GET to /mcp without the streaming headers returns a 4xx, which still
        # proves the socket is bound and serving. Any HTTP response = ready.
