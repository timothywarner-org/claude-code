...
Pricing varies by model. The **current lineup** is **Sonnet 5** (`claude-sonnet-5`), **Opus 4.8** (`claude-opus-4-8`), and **Haiku 4.5** (`claude-haiku-4-5-20251001`):

| Model | API ID | Input (per 1M tokens) | Output (per 1M tokens) |
| ----- | ------ | --------------------- | ---------------------- |
| **Opus 4.8** | `claude-opus-4-8` | $5.00 | $25.00 |
| **Sonnet 5** | `claude-sonnet-5` | Confirm at pricing page | Confirm at pricing page |
| **Haiku 4.5** | `claude-haiku-4-5-20251001` | Confirm at pricing page | Confirm at pricing page |

> Pricing moves. Confirm the current per-model rates at the [Anthropic pricing page](https://www.anthropic.com/pricing) before you record a cost figure. The Opus 4.8 rate above is the one this exercise's model-selection code uses.
...
function selectOptimalModel(task: string, inputTokens: number): string {
  ...
  // Very large context or heavy agentic coding: Opus 4.8 (1M context, $5/$25).
  if (inputTokens > 200_000 || task === 'agentic-coding') {
    return 'claude-opus-4-8';
  }

  // Default to Sonnet 5 - best speed-to-intelligence ratio, 1M context.
...
function withCostTracking<T>(
  ...
  return fn().then((result) => {
    const cost = calculateCost('claude-sonnet-5', result.usage.input_tokens, result.usage.output_tokens);
